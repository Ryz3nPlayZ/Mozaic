# Mozaic Last.fm Proxy Worker

A lightweight [Cloudflare Worker](https://developers.cloudflare.com/workers/) that proxies Last.fm API requests for the Mozaic macOS app. The app sends unsigned requests; the Worker adds `api_key` and computes `api_sig` (MD5) before forwarding to Last.fm.
