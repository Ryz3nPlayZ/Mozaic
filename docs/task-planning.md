...
> 🔁 **Follow the repo default loop in `AGENTS.md`** — Local verification is CLI-first: `swift build`, targeted `swift test --skip MozaicUITests`, then `swiftlint --strict && swiftformat .`. Use Xcode/`xcodebuild` only for UI/runtime debugging, scheme-specific investigation, or CI parity.
...
**Exit gate**: Targeted `swift test --skip MozaicUITests --filter ...` passes for the new code path.
...
| Full default test suite passes | `swift test --skip MozaicUITests` succeeds |

**Exit gate**: `swift build`, `swift test --skip MozaicUITests`, `swiftlint --strict`, and `swiftformat .` pass locally.
...
├── Exit: `swift test --skip MozaicUITests --filter NewServiceTests` passes

Phase 4: QA
├── Run `swift build`
├── Run `swift test --skip MozaicUITests`
...
- **Reference existing code for consistency** — When building features similar to existing ones, explicitly find and match existing implementations. A new detail view should match the patterns in existing detail views. A new parser should follow the structure in `Sources/Mozaic/Services/API/Parsers/`. Point to the reference file and replicate its patterns rather than designing from scratch.
