...
Mozaic provides keyboard control for playback and navigation while preserving standard macOS window shortcuts.
...
Mute is still available from the Playback menu and AppleScript, but Mozaic intentionally does not assign a default mute shortcut so the native macOS minimize shortcut (`⌘M`) continues to work.
