...
Mozaic can present two parallel experiences over the same Google login: the
...
1. **Observer script** — posts to the `youtubePlayer` message handler:
   ...
   It also enforces the Mozaic volume target (`window.__mozaicTargetVolume`)
   and disables YouTube's autonav toggle so Mozaic stays in control.
2. **Extraction script** — hides all page chrome with an ancestor-chain
   ...
   `.mozaic-visible` chain from `#movie_player video` to the root, enforced
   per-frame while active. Defines `window.__mozaicExtractVideo()` so
...
whenever Mozaic's volume target is audible (YouTube persists its own mute
...
- Navigating away within YouTube while **playing** pops the surface out
  ...
  `mozaic://` URL scheme open the floating window regardless.
...
MozaicApp observes `surfaceLocation` and opens/closes the floating window;
...
Mozaic does not block ads. During an ad, `STATE_UPDATE.isAd` is true and
...
- Parser tests run against sanitized captured fixtures in
  `Tests/MozaicTests/Fixtures/YouTube/` (captured via
...
- No auto-advance to the next related video after `VIDEO_ENDED` (YouTube
  autonav is disabled; Mozaic shows the ended state — the bar's next
