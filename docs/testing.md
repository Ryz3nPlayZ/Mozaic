...
This document covers testing strategies, commands, and best practices for Mozaic.

> 🔁 **Default local loop lives in `AGENTS.md`** — Stay CLI-first for everyday verification: `swift build`, `swift test --skip MozaicUITests`, then `swiftlint --strict && swiftformat .`. Use Xcode/`xcodebuild` only for UI/runtime debugging, scheme-specific investigation, or CI parity.
...
New code in `Sources/Mozaic/` (Services, Models, ViewModels, Utilities) must include unit tests.

### Creating a Test File

1. Create test file in `Tests/MozaicTests/` matching the source file name
...
swift test --skip MozaicUITests --filter Parser
```

If you need Xcode's logs or scheme-specific filtering, escalate to:

```bash
xcodebuild test -scheme Mozaic -only-testing:MozaicTests -skip-testing:MozaicUITests
```
...
```swift
// Tests/MozaicTests/Helpers/MockYTMusicClient.swift
...
```swift
// Tests/MozaicTests/Helpers/MockYouTubeClient.swift
...
Regular YouTube parser tests should use sanitized fixtures in `Tests/MozaicTests/Fixtures/YouTube/`; re-capture with `swift run api-explorer --youtube ... -o` when YouTube renderer shapes change.
...
```swift
// Tests/MozaicTests/Helpers/MockURLProtocol.swift
MockURLProtocol.requestHandler = { request in
...
```swift
// Tests/MozaicTests/Helpers/TestFixtures.swift
...
xcodebuild test -scheme Mozaic -destination 'platform=macOS' \
  -only-testing:MozaicTests -skip-test-tag integration

# Scheduled job: Run integration tests (may need re-runs)
xcodebuild test -scheme Mozaic -destination 'platform=macOS' \
  -only-testing:MozaicTests/MusicIntentIntegrationTests
```
...
```
subsystem:Mozaic category:player
subsystem:Mozaic category:auth
```
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build
        run: xcodebuild -scheme Mozaic -destination 'platform=macOS' build

      - name: Test
        run: xcodebuild -scheme Mozaic -destination 'platform=macOS' test
