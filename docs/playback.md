...
This document details Mozaic's YouTube Music WebView-based playback system, its architecture, and implementation notes. Regular YouTube video playback is a separate source with its own `YouTubePlayerService` and `YouTubeWatchWebView`; see [YouTube Mode](youtube.md).
...
Mozaic's queue or whether YouTube autoplay needs to be corrected.
...
queue after Mozaic has already loaded the next track.
...
func loadVideo(videoId: String) {
    ...
    webView.evaluateJavaScript("document.querySelector('video')?.pause()") { _, _ in
        webView.evaluateJavaScript("window.__mozaicTargetVolume = currentVolume")
        self.webView?.load(URLRequest(url: watchURL))
    }
}
```

When the WebView reports a new `videoId`, Mozaic treats that as authoritative
...
Mozaic injects a small set of playback preference scripts into the singleton
...
| Playback audio-quality bootstrap | document start | Stores the preferred `SettingsManager.PlaybackAudioQuality` value on `window.__mozaicPlaybackAudioQuality` before YouTube's player finishes booting. |
...
| Playback audio-quality sync | runtime preference changes | Updates `window.__mozaicPlaybackAudioQuality` and immediately reapplies the preference if the override script is already installed. |
...
```bash
/usr/bin/log show --process Mozaic --last 10m --style compact --info --debug \
  | grep 'Audio quality stats'
```

For a narrower query against Mozaic's unified logging subsystem:

```bash
/usr/bin/log show --process Mozaic --last 10m --style compact --info --debug \
  --predicate 'subsystem == "com.zemuliu.Mozaic"' \
  | grep 'Audio quality stats'
```
...
| Field | Meaning |
|-------|---------|
| `preferred` | Mozaic's selected playback audio-quality setting. |
| `desired` | The string Mozaic sent to YouTube player APIs. |
...
Mozaic treats its own queue as the source of truth whenever one exists:
...
- If YouTube advances to an unexpected track near the end of a song, Mozaic
  replays the expected queue track instead of inheriting YouTube autoplay
- At the end of a non-repeating queue, Mozaic marks playback ended rather than
