...
|-----------|------|---------|
| `VideoWindowController` | `Sources/Mozaic/VideoWindowController.swift` | Manages the floating video window lifecycle |
| `VideoPlayerWindow` | `Sources/Mozaic/Views/VideoPlayerWindow.swift` | SwiftUI view for video display |
| `VideoContainerView` | `Sources/Mozaic/Views/VideoPlayerWindow.swift` | NSView container that hosts the WebView |
| `SingletonPlayerWebView+VideoMode` | `Sources/Mozaic/Views/SingletonPlayerWebView+VideoMode.swift` | CSS injection for video extraction |
...
3. **Create Container**: Creates `#mozaic-video-container` with `position: fixed`
...
1. `window.__mozaicTargetVolume` stores the app's target volume
...
**Solution**: Inject a blackout overlay (`#mozaic-blackout`) immediately when entering video mode. This black `<div>` covers the entire viewport until video extraction is complete.
...
3. Observer script applies `__mozaicTargetVolume` when video element is first detected
...
**Files**:
- [Sources/Mozaic/Models/MusicVideoType.swift](../Sources/Mozaic/Models/MusicVideoType.swift) — Enum definition with `hasVideoContent` property
- [Sources/Mozaic/Services/API/Parsers/SongMetadataParser.swift](../Sources/Mozaic/Services/API/Parsers/SongMetadataParser.swift) — `parseMusicVideoType(from:)` method
- [Sources/Mozaic/Services/Player/PlayerService+Library.swift](../Sources/Mozaic/Services/Player/PlayerService+Library.swift) — Calls `updateVideoAvailability(hasVideo:)` after parsing
...
**File**: [Sources/Mozaic/Views/SingletonPlayerWebView+ObserverScript.swift](../Sources/Mozaic/Views/SingletonPlayerWebView+ObserverScript.swift)
