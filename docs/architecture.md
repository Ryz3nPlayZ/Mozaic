...
This document provides detailed information about Mozaic's architecture, services, and design patterns. Mozaic now has two parallel content sources over the same Google login: YouTube Music (`.music`, default) and regular YouTube (`.video`). Shared infrastructure handles auth, settings, caching, images, and media keys; source-specific clients, models, parsers, view models, and playback WebViews keep the two experiences isolated.
...
```
Sources/
  └── Mozaic/            → Main app target
      ├── MozaicApp.swift    → App entry point
...
```swift
// Sources/Mozaic/Services/Protocols.swift
protocol YTMusicClientProtocol: Sendable { ... }
protocol AuthServiceProtocol { ... }
protocol PlayerServiceProtocol { ... }

// Sources/Mozaic/Services/YouTubeProtocols.swift
...
YouTube view models follow the same pattern with `YouTubeClientProtocol` and live under `Sources/Mozaic/ViewModels/YouTube/`. `YouTubeViewModelStore` keeps the YouTube navigation stack and view-model caches warm while the user switches back to Music.
...
**File**: `Sources/Mozaic/Services/WebKit/WebKitManager.swift`
...
**File**: `Sources/Mozaic/Services/Auth/AuthService.swift`
...
**File**: `Sources/Mozaic/Services/API/YTMusicClient.swift`
...
**File**: `Sources/Mozaic/Services/API/YouTubeClient.swift`
...
**Directory**: `Sources/Mozaic/Services/API/Parsers/`
...
**File**: `Sources/Mozaic/Services/Player/PlayerService.swift`
...
**File**: `Sources/Mozaic/Views/MiniPlayerWebView.swift`
...
**Files**:
- `Sources/Mozaic/Services/Player/YouTubePlayerService.swift`
- `Sources/Mozaic/Views/YouTube/YouTubeWatchWebView.swift`
...
**File**: `Sources/Mozaic/Services/Player/PlaybackArbiter.swift`
...
**File**: `Sources/Mozaic/Services/Player/NowPlayingManager.swift`
...
**File**: `Sources/Mozaic/Services/HapticService.swift`
...
**File**: `Sources/Mozaic/Services/FavoritesManager.swift`
...
**Persistence**:
- **Location**: `~/Library/Application Support/Mozaic/favorites.json`
...
**Related Files**:
- `Sources/Mozaic/Models/FavoriteItem.swift` — Data model with `ItemType` enum
- `Sources/Mozaic/Views/SharedViews/FavoritesSection.swift` — Horizontal scrolling UI
- `Sources/Mozaic/Views/SharedViews/FavoritesContextMenu.swift` — Shared context menu items

### NotificationService

**File**: `Sources/Mozaic/Services/Notification/NotificationService.swift`
...
**File**: `Sources/Mozaic/Services/NetworkMonitor.swift`
...
**File**: `Sources/Mozaic/Services/SettingsManager.swift`
...
**File**: `Sources/Mozaic/Services/Audio/EqualizerService.swift`
...
> ⚠️ **Naming note**: `Sources/Mozaic/Views/SharedViews/EqualizerView.swift` is unrelated — it's a small now-playing animation indicator (three bouncing bars). The settings UI is `EqualizerSettingsView`.

### SyncedLyricsService

**File**: `Sources/Mozaic/Services/Lyrics/SyncedLyricsService.swift`
...
**Related Files**:
- `Sources/Mozaic/Services/Lyrics/LyricsProvider.swift` — Provider protocol and search model
- `Sources/Mozaic/Services/Lyrics/Providers/LRCLibProvider.swift` — External synced lyrics provider
- `Sources/Mozaic/Services/API/Parsers/LRCParser.swift` — LRC to `SyncedLyrics` parser

**Integration**: Created once in `MozaicApp` and injected through the SwiftUI environment for lyrics views.

See [ADR-0012: Synced Lyrics Provider Architecture](adr/0012-synced-lyrics-architecture.md) for design details.

### ShareService

**File**: `Sources/Mozaic/Services/ShareService.swift`
...
**File**: `Sources/Mozaic/Services/SongLikeStatusManager.swift`
...
**File**: `Sources/Mozaic/Services/URLHandler.swift`

Parses YouTube Music URLs, regular YouTube watch links, and custom `mozaic://` URLs:
...
| `mozaic://play?v=xxx` | `.song(videoId:)` |
| `mozaic://playlist?list=xxx` | `.playlist(id:)` |
| `mozaic://album?id=xxx` | `.album(id:)` |
| `mozaic://artist?id=xxx` | `.artist(id:)` |

**Usage**: Called from `MozaicApp.onOpenURL` to handle deep links. Music song links play through `PlayerService`; regular YouTube watch links switch to the YouTube source and open playback in the floating video window.

### ScriptCommands (AppleScript)

**File**: `Sources/Mozaic/Services/Scripting/ScriptCommands.swift`
...
**Related Files**:
- `Sources/Mozaic/Mozaic.sdef` — AppleScript dictionary definition
- `Sources/Mozaic/Info.plist` — `NSAppleScriptEnabled` and `OSAScriptingDefinition` keys

**Usage**:
```applescript
tell application "Mozaic"
    play
    set volume 50
...
**File**: `Sources/Mozaic/Services/UpdaterService.swift`
...
**File**: `Sources/Mozaic/AppDelegate.swift`
...
**File**: `Sources/Mozaic/Models/YTMusicError.swift`
...
The `Sources/Mozaic/Utilities/` folder contains shared helper components.

### ImageCache

**File**: `Sources/Mozaic/Utilities/ImageCache.swift`
...
**File**: `Sources/Mozaic/Utilities/ColorExtractor.swift`
...
**File**: `Sources/Mozaic/Utilities/AnimationConfiguration.swift`
...
**File**: `Sources/Mozaic/Utilities/AccessibilityIdentifiers.swift`
...
**File**: `Sources/Mozaic/Utilities/IntelligenceModifier.swift`
...
**File**: `Sources/Mozaic/Utilities/RetryPolicy.swift`
...
**File**: `Sources/Mozaic/Services/API/APICache.swift`
...
Mozaic integrates Apple's on-device Foundation Models framework for AI-powered features. See [ADR-0005: Foundation Models Architecture](adr/0005-foundation-models-architecture.md) for detailed design decisions.
...
|-----------|----------|---------|
| `FoundationModelsService` | `Sources/Mozaic/Services/AI/` | Singleton managing AI availability and sessions |
| `@Generable` Models | `Sources/Mozaic/Models/AI/` | Type-safe structured outputs (`MusicIntent`, `LyricsSummary`, etc.) |
| Tools | `Sources/Mozaic/Services/AI/Tools/` | Ground AI responses in real catalog data |
| `AIErrorHandler` | `Sources/Mozaic/Services/AI/` | User-friendly error messages |
| `RequiresIntelligenceModifier` | `Sources/Mozaic/Utilities/` | Hide AI features when unavailable |

### AI Tools

Tools ground AI responses in real data from the YouTube Music catalog, preventing hallucination.

**MusicSearchTool** (`Sources/Mozaic/Services/AI/Tools/MusicSearchTool.swift`)
...
**QueueTool** (`Sources/Mozaic/Services/AI/Tools/QueueTool.swift`)
...
| Model | File | Purpose |
|-------|------|---------|
| `MusicIntent` | `Sources/Mozaic/Models/AI/MusicIntent.swift` | Parsed user commands (play, pause, search, etc.) |
| `LyricsSummary` | `Sources/Mozaic/Models/AI/LyricsSummary.swift` | Lyrics explanation with themes and mood |
| `PlaylistChanges` | `Sources/Mozaic/Models/AI/PlaylistChanges.swift` | Queue refinement operations |
...
**File**: `Sources/Mozaic/Views/QueueView.swift`
...
**File**: `Sources/Mozaic/Views/LyricsView.swift`
...
**File**: `Sources/Mozaic/Views/CommandBarView.swift`
...
**File**: `Sources/Mozaic/Views/ToastView.swift`
...
**Directory**: `Sources/Mozaic/Views/SharedViews/`
...
**File**: `Sources/Mozaic/Views/PlayerBar.swift`
...
**File**: `Sources/Mozaic/Views/Sidebar.swift`
