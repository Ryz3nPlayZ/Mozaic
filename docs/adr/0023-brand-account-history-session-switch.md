...
- **History is recorded by the playback WebView, not by Mozaic.** A repo
  grep for `watchtime|videostats|/api/stats|playbackTracking` over
  `Sources/Mozaic` returns zero hits. Real-world history is created by the
...
- **Per-request brand override cannot fix playback.** Verified live: a
  ...
  Mozaic plays in a WebView at all). So recording **must** stay in the
