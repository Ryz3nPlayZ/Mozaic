...
Mozaic's command bar had already moved to a local-first architecture, but the AI path still used a broad `MusicIntent` schema as its first contract. That left two issues:
...
- the settings UI exposed a "Clear AI Context" control even though Mozaic creates fresh Foundation Models sessions per request and had no persistent session context to clear

## Decision

Mozaic now uses a two-stage command-bar AI contract:
...
- the settings UI is more honest about Mozaic's stateless session model
