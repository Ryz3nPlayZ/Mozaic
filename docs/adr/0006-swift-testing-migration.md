...
**Files to create:**
- `Tests/MozaicTests/SwiftTestingHelpers/Tags.swift` — Define custom test tags
...
1. **Split `MozaicTests.swift`** before migration:
   ...
   - Delete `MozaicTests.swift` once empty
...
**Files in `MozaicUITests/`:**
- `MozaicUITestCase.swift` (base class)
...
| Task | Status |
|------|--------|
| Split `MozaicTests.swift` into proper test files | ✅ Done |
| Delete `MozaicTests.swift` after split | ✅ Done |
| Verify no mocks use `static var` | ✅ Done |

### Unit Tests (`MozaicTests/`)
...
```swift
// Tests/MozaicTests/SwiftTestingHelpers/Tags.swift
import Testing
...
xcodebuild -scheme Mozaic -destination 'platform=macOS' test

# Run only unit tests (excludes UI tests)
xcodebuild -scheme Mozaic -destination 'platform=macOS' test -only-testing:MozaicTests
...
1. **Create `Tags.swift`** in `Tests/MozaicTests/SwiftTestingHelpers/`
2. **Split `MozaicTests.swift`** into proper test files
3. **Migrate `LikeStatusTests.swift`** as the pilot (simplest file)
4. **Verify both frameworks run together**: `xcodebuild test -only-testing:MozaicTests`
