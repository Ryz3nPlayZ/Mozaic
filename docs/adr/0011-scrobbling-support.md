...
Mozaic currently has no way to report listening activity to external services. Users who use Last.fm for tracking their music history cannot get scrobbles from Mozaic. Adding scrobbling support requires:
...
3. **`ScrobbleQueue`** — A persistent JSON queue (`~/Library/Application Support/Mozaic/scrobble-queue.json`) that ensures every scrobble is written to disk before network submission.
