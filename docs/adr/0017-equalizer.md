...
Users requested an equalizer comparable to Spotify's mobile EQ — six bands, presets, audible effect. Mozaic's audio path makes this non-trivial:
...
4. **Multi-process architecture**: WebKit moved media decode into the **`com.apple.WebKit.GPU`** XPC subprocess on macOS Sonoma+. The Mozaic main process never touches the audio samples.
...
`ProcessTapHelper` enumerates Core Audio's process-object list via `kAudioHardwarePropertyProcessObjectList`, filters to `com.apple.WebKit.GPU` / `com.apple.WebKit.WebContent`, and prefers entries whose parent PID matches Mozaic. Tapping happens with `CATapMuteBehavior.mutedWhenTapped` so WebKit's direct output is silenced and only the EQ-processed render reaches the speakers.
...
- `Mozaic.entitlements`: `com.apple.security.device.audio-input` (sandbox capability for audio capture in the presence of `app-sandbox`).
- macOS 14.2+ runtime check inside `ProcessTapHelper.start()`.

## Consequences

- New code lives under `Sources/Mozaic/Models/` (`EQSettings`, `EQBand`, `EQPreset`), `Sources/Mozaic/Services/Audio/` (`EqualizerService`, `EqualizerAudioEngine`, `ProcessTapHelper`, `BiquadFilter`), and `Sources/Mozaic/Views/EqualizerSettingsView.swift`.
...
- **Other macOS apps' audio is unaffected** — the tap targets only Mozaic's WebKit subprocess.
