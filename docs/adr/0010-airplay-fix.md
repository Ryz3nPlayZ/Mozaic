...
**Issue:** [GitHub Issue #42](https://github.com/sozercan/mozaic/issues/42) - AirPlay does not work
...
1. **`Sources/Mozaic/Views/SingletonPlayerWebView+PlaybackControls.swift`** - Add `showAirPlayPicker()` method
2. **`Sources/Mozaic/Views/PlayerBar.swift`** - Replace `AVRoutePickerView` with custom button
3. **`Sources/Mozaic/Services/Player/PlayerService.swift`** - Add `showAirPlayPicker()` method to call through to SingletonPlayerWebView

### Step 1: Add JavaScript injection method

**File:** `Sources/Mozaic/Views/SingletonPlayerWebView+PlaybackControls.swift`
...
**File:** Wherever `AccessibilityID.PlayerBar` is defined (likely `Sources/Mozaic/Constants/AccessibilityID.swift` or similar)
