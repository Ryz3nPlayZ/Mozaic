...
2. **SwiftPM-first compatibility** — Mozaic builds primarily via SwiftPM, with checked-in Xcode projects for app and UI-test workflows
...
The repo is SwiftPM-first, but it also includes `Mozaic.xcodeproj` and `MozaicUITests.xcodeproj` for app packaging, runtime debugging, and UI-test workflows.
