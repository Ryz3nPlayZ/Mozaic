...
Mozaic did not have a way to install or manage WebKit web extensions. The app already relied on WebKit for authentication and DRM playback, but users could not:
...
3. **Copied into app storage** — Mozaic clones the selected extension into `~/Library/Application Support/Mozaic/ManagedExtensions/` so it can load a stable local copy.
4. **Persisted metadata** — the extension list is stored as JSON in `~/Library/Application Support/Mozaic/extensions.json`, including the copied folder path plus any options or popup page metadata derived from `manifest.json`.
