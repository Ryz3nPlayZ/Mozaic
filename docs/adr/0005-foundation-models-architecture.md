...
Mozaic needed AI-powered features for natural language music control, lyrics analysis, and queue management. Apple's Foundation Models framework (macOS 26+) provides on-device AI capabilities that align with our privacy-first approach and integrate seamlessly with the Apple ecosystem.
...
| Model | Purpose | File |
|-------|---------|------|
| `MusicIntent` | Parse natural language commands (play, queue, shuffle) | `Sources/Mozaic/Models/AI/MusicIntent.swift` |
| `MusicQuery` | Structured search queries with filters | `Sources/Mozaic/Models/AI/MusicQuery.swift` |
| `QueueIntent` | Queue-specific commands (add, remove, reorder) | `Sources/Mozaic/Models/AI/QueueIntent.swift` |
| `QueueChanges` | AI-suggested queue modifications | `Sources/Mozaic/Models/AI/QueueChanges.swift` |
| `LyricsSummary` | Lyrics analysis (themes, mood, explanation) | `Sources/Mozaic/Models/AI/LyricsSummary.swift` |
| `PlaylistChanges` | Playlist refinement suggestions | `Sources/Mozaic/Models/AI/PlaylistChanges.swift` |
...
| Tool | Purpose | File |
|------|---------|------|
| `MusicSearchTool` | Search YouTube Music catalog | `Sources/Mozaic/Services/AI/Tools/MusicSearchTool.swift` |
| `QueueTool` | Get current queue state | `Sources/Mozaic/Services/AI/Tools/QueueTool.swift` |
| `NowPlayingTool` | Get currently playing track | `Sources/Mozaic/Services/AI/Tools/NowPlayingTool.swift` |
| `PlaylistTool` | Get playlist contents | `Sources/Mozaic/Services/AI/Tools/PlaylistTool.swift` |
| `LibraryTool` | Access user's library | `Sources/Mozaic/Services/AI/Tools/LibraryTool.swift` |
...
Call `prewarm()` API on app launch via `.task` modifier in `MozaicApp.swift`. This loads the model into memory before user interaction.
