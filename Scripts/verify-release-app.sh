#!/usr/bin/env bash
# Verifies the packaged Mozaic.app has the release-critical settings that are
# easy to regress when building outside Xcode.

set -euo pipefail

ROOT=$(cd "$(dirname "$0")/.." && pwd)
APP_PATH="$ROOT/.build/app/Mozaic.app"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<USAGE
Usage: Scripts/verify-release-app.sh [--require-developer-id] [path/to/Mozaic.app]
...
ENTITLEMENTS_PLIST=$(mktemp "${TMPDIR:-/tmp}/mozaic-entitlements.XXXXXX.plist")
CODESIGN_DETAILS=$(mktemp "${TMPDIR:-/tmp}/mozaic-codesign.XXXXXX.txt")
...
expected_bundle_id = "com.zemuliu.Mozaic"
...
if entitlements.get("com.apple.security.app-sandbox") is True:
    ...
    errors.append("Mozaic release app must be sandboxed")
