...
git clone https://github.com/Ryz3nPlayZ/Mozaic.git
cd mozaic
...
MOZAIC_SIGNING=adhoc ./Scripts/compile_and_run.sh
...
| File                                                | Purpose                                    |
| --------------------------------------------------- | ------------------------------------------ |
| `Sources/Mozaic/AppDelegate.swift`                   | Window lifecycle, background audio support |
| `Sources/Mozaic/Services/WebKit/WebKitManager.swift` | Cookie store & persistence                 |
| `Sources/Mozaic/Services/Auth/AuthService.swift`     | Login state machine                        |
| `Sources/Mozaic/Services/API/YTMusicClient.swift`    | YouTube Music API client                   |
| `Sources/Mozaic/Services/API/YouTubeClient.swift`    | Regular YouTube API client                 |
| `Sources/Mozaic/Services/Player/PlayerService.swift` | Playback state & control                   |
| `Sources/Mozaic/Services/Player/YouTubePlayerService.swift` | Regular YouTube playback state & control   |
| `Sources/Mozaic/Views/MiniPlayerWebView.swift`       | Singleton WebView, playback UI             |
| `Sources/Mozaic/Views/YouTube/YouTubeWatchWebView.swift` | Singleton WebView for regular YouTube watch playback |
| `Sources/Mozaic/Views/MainWindow.swift`              | Main app window                            |
| `Sources/Mozaic/Utilities/DiagnosticsLogger.swift`   | Logging                                    |
...
Mozaic has two playback paths. YouTube Music uses `PlayerService` plus the
...
3. **Tests Must Pass** — Run `swift test --skip MozaicUITests` for unit tests
...
Reference: Sources/Mozaic/Services/HapticService.swift for existing patterns
```
...
swift test --skip MozaicUITests --filter PlayerServiceTests
```
