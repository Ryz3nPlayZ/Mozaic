...
/// links, and Mozaic custom-scheme URLs.
///
/// Supports URLs like:
...
/// - `mozaic://play?v=dQw4w9WgXcQ` - Custom scheme for song
/// - `mozaic://playlist?list=PLxxx` - Custom scheme for playlist
/// - `mozaic://album?id=MPRExxx` - Custom scheme for album
/// - `mozaic://artist?id=UCxxx` - Custom scheme for artist
enum URLHandler {
    // MARK: - Types
    ...
    static func parse(_ url: URL) -> ParsedContent? {
        ...
        if url.scheme == "mozaic" {
            return self.parseMozaicURL(url)
        }
    ...
    /// Parses a mozaic:// custom scheme URL.
    private static func parseMozaicURL(_ url: URL) -> ParsedContent? {
        guard url.scheme == "mozaic" else { return nil }
