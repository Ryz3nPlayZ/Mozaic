...
/// equalizer.
///
/// **Critical detail**: Mozaic doesn't decode music itself — `WKWebView` does,
...
final class ProcessTapHelper {
    ...
    private static let tapName = "com.zemuliu.Mozaic.EQ.Tap"

    /// User-visible name of the aggregate device that wraps the tap.
    private static let aggregateName = "Mozaic EQ Aggregate"
    ...
    private static let aggregateUIDPrefix = "com.zemuliu.Mozaic.EQ.Aggregate."
    ...
    func start() -> Result<Void, StartFailure> {
        ...
        // they show up in Audio MIDI Setup as lingering "Mozaic EQ Aggregate"
    ...
    ///
    /// Mozaic's audio is decoded by `WKWebView`'s WebKit subprocesses, not
    ...
    private static func audioObjectsToTap() -> [AudioObjectID] {
        ...
        if !candidates.isEmpty {
            Self.logger.warning(
                "found \(candidates.count) WebKit audio process(es) but none were provably owned by Mozaic; skipping tap to avoid hijacking unrelated audio"
            )
        }
        return []
    }
    ...
    ///
    /// WebKit helper names include the host app ("Mozaic Web Content",
    /// "Mozaic Graphics and Media"), which survives even when their Unix
    ...
    /// "Mozaic Networking", which provides a second ownership proof path.
