...
/// HAL-level duplex implementation of the Mozaic equalizer.
///
...
final class EqualizerAudioEngine: EqualizerAudioEngineProtocol {
    ...
    func start() -> Result<Void, StartFailure> {
        ...
        let createStatus = AudioDeviceCreateIOProcID(
            aggregateID, mozaicEQIOProc, selfRef, &procID
        )
    ...
    enum StartFailure: Error {
        ...
        var userFacingMessage: String {
            ...
            case let .tap(.tapCreation(status)):
                String(localized: "Couldn't capture Mozaic's audio (status \(status)). Check Screen & System Audio Recording permission in System Settings.")
            case .tap(.permissionDenied):
                String(localized: "Open System Settings → Privacy & Security → Screen & System Audio Recording and enable Mozaic, then toggle the equalizer on again.")
            case .tap(.aggregateDeviceCreation):
                String(localized: "Couldn't create the equalizer audio device. Restarting Mozaic usually fixes this.")
            case .tap(.unsupportedOS):
                String(localized: "The equalizer requires macOS 14.2 or later.")
            case .invalidTapFormat:
                String(localized: "The system didn't report a valid audio format for Mozaic's output. Try disabling the equalizer, starting playback, then enabling it again.")
