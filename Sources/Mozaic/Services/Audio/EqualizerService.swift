...
/// (typically wired to `PlayerService.isPlaying` from `MozaicApp`), the
...
final class EqualizerService {
    ...
    /// coupling minimal — `MozaicApp` wires `PlayerService.isPlaying` in.
    ...
    /// engine isn't running yet — typically called by `MozaicApp` when
    ...
    var status: Status {
        ...
        if self.inferredPermissionDenial {
            return .permissionNeeded(message: String(
                localized: "Open System Settings → Privacy & Security → Screen & System Audio Recording and enable Mozaic, then retry playback or toggle the equalizer off and on."
            ))
        }
