...
extension PlayerService {
    ...
    private func isRepeatAllWraparoundTrackEnd(
        ...
        // observed id before the natural `ended` callback reaches Mozaic.
    ...
    private func handleMozaicInitiatedPlaybackMetadata(
        observedVideoId: String?,
        title: String,
        artist: String,
        thumbnailUrl: String,
        ...
        guard self.isMozaicInitiatedPlayback, !self.queue.isEmpty else {
            return false
        }
        ...
        if self.observedTrackMatchesSong(
            ...
            self.isMozaicInitiatedPlayback = false
            self.logger.debug("Confirmed Mozaic-initiated playback for '\(intendedSong.title)'")
            return false
        }
    ...
    private func replayCurrentQueueSongForRepeatOneAfterTrackEnd() async {
        ...
        self.songNearingEnd = false
        let mozaicAlignedWithQueue = self.pendingPlayVideoId == currentSong.videoId
            && SingletonPlayerWebView.shared.currentVideoId == currentSong.videoId
        if self.hasUserInteractedThisSession, mozaicAlignedWithQueue {
    ...
    /// Updates track metadata and enforces Mozaic's queue when YouTube tries to diverge.
    func updateTrackMetadata(title: String, artist: String, thumbnailUrl: String, videoId observedVideoId: String?) {
        ...
        if self.handleMozaicInitiatedPlaybackMetadata(
            observedVideoId: observedVideoId,
            title: title,
            artist: artist,
            thumbnailUrl: thumbnailUrl,
