...
/// visible so the page can dock into native Mozaic views.
///
...
final class YouTubeWatchWebView {
    ...
    private func load(videoId: String) {
        ...
        webView.evaluateJavaScript("document.querySelector('video')?.pause()") { [weak self] _, _ in
            ...
            webView.evaluateJavaScript("window.__mozaicTargetVolume = \(targetVolume);", completionHandler: nil)
    ...
    nonisolated static func pageBootstrapScript(targetVolume: Double, pendingSeek: Double? = nil) -> String {
        ...
        var script = "window.__mozaicTargetVolume = \(clamped);"
        if let pendingSeek, pendingSeek.isFinite, pendingSeek > 0 {
            script += " window.__mozaicPendingSeek = \(pendingSeek);"
        }
        return script
    }
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        func webView(_ webView: WKWebView, didFinish _: WKNavigation!) {
            ...
            webView.evaluateJavaScript(
                """
                (function() {
                    window.__mozaicTargetVolume = \(savedVolume);
                    const video = document.querySelector('video');
                    if (video) { video.volume = \(savedVolume); }
                    if (window.__mozaicExtractVideo) { window.__mozaicExtractVideo(); }
                })();
                """,
