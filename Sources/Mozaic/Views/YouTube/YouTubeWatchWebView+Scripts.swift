...
extension YouTubeWatchWebView {
            ...
            function disableAutonav() {
                try {
                    ...
                    if (toggle && toggle.getAttribute('aria-checked') === 'true') {
                        toggle.click();
                        console.log('[MozaicYT] Disabled YouTube autonav');
                    }
                } catch (e) {}
            }
    ...
    /// `.mozaic-visible` chain (and caption whitelists) win once applied.
    static var blackoutScript: String {
        """
        (function() {
            'use strict';
            const style = document.createElement('style');
            style.id = 'mozaic-yt-blackout';
            style.textContent = `
