...
struct MiniPlayerWindow: View {
    ...
    private var hoverCommandPill: some View {
        HStack(spacing: 11) {
            self.hoverIconButton(
                ...
                accessibilityID: AccessibilityID.MiniPlayer.returnToMozaicButton,
                label: String(localized: "Return to Mozaic")
            ) {
    ...
    private var artistText: some View {
        Text(self.playerService.currentTrack?.artistsDisplay.isEmpty == false ? self.playerService.currentTrack?.artistsDisplay ?? "" : String(localized: "Mozaic"))
            .lineLimit(1)
