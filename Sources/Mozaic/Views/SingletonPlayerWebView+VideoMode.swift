...
extension SingletonPlayerWebView {
    ...
    func injectBlackoutCSS() {
        ...
        let script = """
            (function() {
                ...
                if (!document.getElementById('mozaic-blackout')) {
                    const blackout = document.createElement('div');
                    blackout.id = 'mozaic-blackout';
            ...
            (function() {
                ...
                if (playerPage && typeof playerPage.videoMode !== 'undefined') {
                    if (playerPage.videoMode !== true) {
                        ...
                        console.log('[Mozaic] Forced videoMode = true via property');
                ...
                if (switcher) {
                    ...
                    if (videoBtn && !videoBtn.hasAttribute('active')) {
                        videoBtn.click();
                        console.log('[Mozaic] Clicked Video toggle via switcher');
                ...
                for (const btn of allButtons) {
                    ...
                    if (text === 'video') {
                        ...
                        if (!isActive) {
                            btn.click();
                            console.log('[Mozaic] Clicked Video button by text');
    ...
    private func removeBlackoutOnly() {
        self.webView?.evaluateJavaScript("const b = document.getElementById('mozaic-blackout'); if (b) b.remove();")
    }
    ...
    private static func videoContainerScriptInPlace() -> String {
        ...
        (function() {
            ...
            const styleId = 'mozaic-video-mode-style-v2';
            ...
            style.textContent = `
                ...
                video.mozaic-visible, .video-stream.mozaic-visible {
            ...
            const markAncestors = () => {
                ...
                // Clear old marks
                document.querySelectorAll('.mozaic-visible').forEach(el => el.classList.remove('mozaic-visible'));
            ...
            const enforceVisibility = () => {
                markAncestors();
                const video = document.querySelector('video');
                if (video && typeof window.__mozaicVideoModeActive === 'boolean' && window.__mozaicVideoModeActive) {
                ...
                if (window.__mozaicVideoModeActive) {
                    requestAnimationFrame(enforceVisibility);
                }
            };
            window.__mozaicVideoModeActive = true;
            requestAnimationFrame(enforceVisibility);

            // Remove the extraction container if it exists from previous attempts
            const oldContainer = document.getElementById('mozaic-video-container');
            if (oldContainer) {
                const video = oldContainer.querySelector('video');
                if (video && video.__mozaicOriginalParent) {
                    video.__mozaicOriginalParent.appendChild(video);
                    delete video.__mozaicOriginalParent;
                }
                oldContainer.remove();
            }
    ...
    func removeVideoModeCSS() {
        ...
        let script = """
            (function() {
                ...
                const blackout = document.getElementById('mozaic-blackout');
                if (blackout) blackout.remove();

                // Remove the in-place style
                const style = document.getElementById('mozaic-video-mode-style-v2');
                if (style) style.remove();

                // Stop the enforcement loop
                window.__mozaicVideoModeActive = false;
