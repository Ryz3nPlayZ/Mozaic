...
//  Reads cookies from the Mozaic app's debug cookie export for authenticated requests.
//
//  Usage:
...
let apiKeyEnvironmentVariable = "MOZAIC_YTMUSIC_API_KEY"
...
/// Reads cookies from Mozaic app's backup file in Application Support.
...
func loadCookiesFromAppBackup() -> [HTTPCookie]? {
    ...
    let cookieFile =
        appSupport
            .appendingPathComponent("Mozaic", isDirectory: true)
...
func buildContext(brandAccountId: String? = nil) -> [String: Any] {
    ...
    // Diagnostic decoupling: set MOZAIC_PROBE_NO_OBOU=1 to omit the body
    ...
    if let brandId = brandAccountId ?? globalBrandAccountId,
       ProcessInfo.processInfo.environment["MOZAIC_PROBE_NO_OBOU"] != "1"
    {
...
func exploreBrowse(
    ...
    do {
        ...
        if statusCode == 401 || statusCode == 403 {
            ...
            print("   Run the Mozaic app and sign in, then try again.")
            return
        }
...
func checkAuthStatus() {
    ...
    guard let cookies = loadCookiesFromAppBackup() else {
        ...
        print("  1. Run the Mozaic app")
        print("  2. Sign in to YouTube Music")
        print("  3. The app will save cookies to ~/Library/Application Support/Mozaic/")
...
func discoverAccounts(verbose: Bool) async {
    ...
    guard loadCookiesFromAppBackup() != nil else {
        print("❌ No cookies found. Please sign in to Mozaic first.")
        return
    }
...
func probeSigninSwitch(brandId: String, authUserIndex: Int, nextURLString: String) async {
    ...
    guard let cookies = loadCookiesFromAppBackup(), !cookies.isEmpty else {
        print("❌ No app cookies found. Sign in to Mozaic first.")
        return
    }
...
func showHelp() {
    print(
        ...
        Supports public and authenticated endpoints (reads cookies from Mozaic app).
        ...
        Examples:
          ...
          # Explore authenticated endpoints (requires Mozaic sign-in)
          ...
          swift run api-explorer auth
            ...
            Authentication:
                ...
                Debug builds export auth cookies to:
                    ~/Library/Application Support/Mozaic/cookies.dat
