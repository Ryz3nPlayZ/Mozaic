...
struct AutoplayRecoveryJSTests {
    private func makeContext() -> JSContext {
        ...
        // loading the recovery function (which references `window.__mozaic…`).
    ...
    func clicksButtonWhenFlagPendingAndPaused() {
        ...
        ctx.evaluateScript(
            ...
            globalThis.result = __mozaicAttemptAutoplayRecovery(video, btn);
            """
        )
        ...
        #expect(ctx.evaluateScript("window.__mozaicAutoplayPending").toBool() == false)
    }
    ...
    func fallsBackToVideoPlay() {
        ...
        ctx.evaluateScript(
            ...
            var played = false;
            var video = { paused: true, play: function() { played = true; } };
            globalThis.result = __mozaicAttemptAutoplayRecovery(video, null);
            """
        )
    ...
    func observerScriptEmbedsRecoveryFunction() {
        #expect(SingletonPlayerWebView.observerScript.contains("__mozaicAttemptAutoplayRecovery"))
    }

    @Test("Observer script clears autoplay intent on successful playback")
    func observerScriptClearsAutoplayIntentOnPlayback() {
        #expect(SingletonPlayerWebView.observerScript.contains("window.__mozaicAutoplayPending = false;"))
    }
...
struct AutoplayIntentScriptTests {
    ...
    func setsPendingTrue() {
        ...
        #expect(script == "window.__mozaicAutoplayPending = true;")
    }
    ...
    func clearsPendingForRestoredSession() {
        ...
        #expect(script == "window.__mozaicAutoplayPending = false;")
    }
    ...
    func pageBootstrapSeedsIntentAndTargetVolume() {
        ...
        #expect(script.contains("window.__mozaicAutoplayPending = true;"))
        #expect(script.contains("window.__mozaicTargetVolume = 0.42;"))
    }
    ...
    func pageBootstrapClampsInvalidTargetVolume() {
        ...
        #expect(script.contains("window.__mozaicTargetVolume = 1.0;"))
    }
}
