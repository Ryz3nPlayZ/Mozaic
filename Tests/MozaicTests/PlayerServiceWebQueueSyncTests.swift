...
struct PlayerServiceWebQueueSyncTests {
    ...
    @Test("UpdateTrackMetadata corrects YouTube autoplay with Mozaic-initiated playback")
    func updateTrackMetadataCorrectsYouTubeAutoplay() async {
        ...
        // Simulate calling next which sets isMozaicInitiatedPlayback
        await self.playerService.next()

        // Get the song that Mozaic intended to play
    ...
    func updateTrackMetadataKeepsQueueSongWhenMetadataIsStale() async {
        ...
        #expect(self.playerService.isMozaicInitiatedPlayback == false)
    }
